 ...
 */
#include <png.h>
void Screen_Grab(void) {
	png_structp png_ptr = NULL;
	png_infop info_ptr = NULL;
	png_text pngtext;
	
	uint8_t *fb;
	uint8_t buf[NeXT_SCRN_WIDTH*NeXT_SCRN_HEIGHT*4];
	...
	char key[] = "Title";
	char text[] = "Previous screenshot";

	FILE *fp = NULL;
	char *szFileName = malloc(FILENAME_MAX);
	
	snprintf(szFileName, FILENAME_MAX, "%s/test.png", ConfigureParams.Printer.szPrintToFileName);
	...
	png_set_IHDR(png_ptr, info_ptr, NeXT_SCRN_WIDTH, NeXT_SCRN_HEIGHT, 8, PNG_COLOR_TYPE_RGB_ALPHA,
	...
	if (ConfigureParams.Screen.nMonitorType==MONITOR_TYPE_DIMENSION) {
		...
		j = 16*4;
#endif
		for (i = 0; i < (NeXT_SCRN_WIDTH*NeXT_SCRN_HEIGHT*4); i+=4) {		
			buf[i+0] = fb[i+j+0]; // r
			buf[i+1] = fb[i+j+1]; // g
			buf[i+2] = fb[i+j+2]; // b
			buf[i+3] = 0xff;      // a
			
			if (i && (i%(NeXT_SCRN_WIDTH*4))==0)
				j+=32*4;
		}
	} else {
		if (NEXTVideo) {
			...
			if (ConfigureParams.System.bColor) {
				for (i = 0; i < (NeXT_SCRN_WIDTH*NeXT_SCRN_HEIGHT*4); i+=4) {
					if (!ConfigureParams.System.bTurbo && i && (i%(NeXT_SCRN_WIDTH*4))==0)
						j+=32*2;
				...
				for (i = 0; i < (NeXT_SCRN_WIDTH*NeXT_SCRN_HEIGHT/4); i++) {
					if (!ConfigureParams.System.bTurbo && i && (i%(NeXT_SCRN_WIDTH/4))==0)
						j+=32/4;
	...
	{		
		src_ptr = buf + y * NeXT_SCRN_WIDTH * 4;
